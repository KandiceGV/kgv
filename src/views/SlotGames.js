...
function importAllSlotGames(r) {
  return slotGames.map(r);
}

const imagesSlotGames = importAllSlotGames(
  require.context("./images/slot_games_providers", false, /\.(png|jpe?g|svg)$/)
);
...
const SlotGames = () => {
    ...
    onCompleted: async (the_data) => {
      ...
      if (the_query_games_categories !== null) {
        ...
        for (let b = 0; b < the_query_games_categories.length; b++) {
          ...
          if (slotGamesCategories.indexOf(the_category_title.toLowerCase()) === -1) {
            continue;
          }
          ...
          if (the_category_banner_img !== undefined) {
            ...
            if (all_the_games[the_index][0] !== undefined) {
              for (let jj = 0; jj < imagesSlotGames.length; jj++) {
                if (
                  imagesSlotGames[jj]
                    .toLowerCase()
                    .includes(the_category_title.toLowerCase())
                ) {
                  game_categories_images.push(imagesSlotGames[jj]);
                  break;
                }
              }
            }
  ...
  const StoreInLocalStorageGoldValley = (index) => {
    ...
    localStorage.setItem("PageThatCameFrom", "slotgames");
  };
