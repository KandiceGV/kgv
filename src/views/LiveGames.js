...
const imagesLiveTables = importAllSlotGames(
  require.context("./images/live_tables_providers", false, /\.(png|jpe?g|svg)$/)
);
        ...
        for (let b = 0; b < the_query_games_categories.length; b++) {
          ...
          if (the_category_banner_img !== undefined) {
            ...
            if (all_the_games[the_index][0] !== undefined) {
              for (let jj = 0; jj < imagesLiveTables.length; jj++) {
                if (
                  imagesLiveTables[jj]
                    .toLowerCase()
                    .includes(the_category_title.toLowerCase())
                ) {
                  game_categories_images.push(imagesLiveTables[jj]);
                  break;
                }
              }
            }
